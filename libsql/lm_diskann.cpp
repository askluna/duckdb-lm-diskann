...
const LmDiskannEdgeType LMDISKANN_DEFAULT_EDGE_TYPE =
    LmDiskannEdgeType::SAME_AS_NODE;
...
constexpr idx_t NODE_VECTOR_ALIGNMENT =
    8; // Align vectors to 8 bytes (adjust if needed)
...
constexpr idx_t DELETE_QUEUE_ENTRY_SIZE =
    sizeof(row_t) + sizeof(block_id_t) + sizeof(uint32_t);
...
idx_t LmDiskannIndex::GetVectorTypeSizeBytes(LmDiskannVectorType type) {
  ...
  default:
    throw InternalException(
        "Unsupported LmDiskannVectorType for size calculation");
  }
}

idx_t LmDiskannIndex::GetEdgeVectorTypeSizeBytes(
    LmDiskannEdgeType type, LmDiskannVectorType node_type) {
  ...
  if (type == LmDiskannEdgeType::SAME_AS_NODE) {
    ...
    switch (type) {
    case LmDiskannEdgeType::FLOAT32:
      resolved_type = LmDiskannVectorType::FLOAT32;
      break;
    case LmDiskannEdgeType::FLOAT16:
      resolved_type = LmDiskannVectorType::FLOAT16;
      break;
    case LmDiskannEdgeType::INT8:
      resolved_type = LmDiskannVectorType::INT8;
      break;
    case LmDiskannEdgeType::FLOAT1BIT:
      return 1; // Special case: 1 bit per dimension (needs careful handling)
    default:
      throw InternalException(
          "Unsupported LmDiskannEdgeType for size calculation");
    }
  }
...
void LmDiskannIndex::ParseOptions(const case_insensitive_map_t<Value> &options,
  ...
  if (it != options.end()) {
    ...
    if (metric_str == "L2") {
      ...
      throw BinderException("Unsupported METRIC type '%s' for LM_DISKANN "
  ...
  if (it != options.end()) {
    ...
    if (edge_type_str == "FLOAT32") {
      ...
      throw BinderException(
          "Unsupported EDGE_TYPE '%s' for LM_DISKANN index. Supported types: "
          "FLOAT32, FLOAT16, INT8, FLOAT1BIT",
          edge_type_str);
    }
  }
...
LmDiskannIndex::LmDiskannIndex(
    ...
    : BoundIndex(name, LmDiskannIndex::TYPE_NAME, index_constraint_type,
      ...
      format_version_(
          LMDISKANN_CURRENT_FORMAT_VERSION), // Set current format version
                                           ...
                                           // invalid
{
  if (index_constraint_type != IndexConstraintType::NONE) {
    throw NotImplementedException(
        "LM_DISKANN indexes do not support UNIQUE or PRIMARY KEY constraints");
  }
  if (logical_types.size() != 1) {
    throw BinderException(
        "LM_DISKANN index can only be created over a single column.");
  }
  if (logical_types[0].id() != LogicalTypeId::ARRAY) {
    throw BinderException(
        "LM_DISKANN index can only be created over ARRAY types.");
  }
  ...
  if (dimensions_ == 0) {
    throw BinderException(
        "Cannot create LM_DISKANN index on ARRAY with unknown size.");
  }
  ...
  if (dimensions_ > MAX_VECTOR_SZ) {
    throw BinderException(
        "Cannot create LM_DISKANN index on ARRAY with dimensions > %d",
        MAX_VECTOR_SZ);
  }
  ...
  default:
    throw BinderException("Unsupported vector type for LM_DISKANN index: %s. "
                          "Supported types: FLOAT[], TINYINT[], FLOAT16[]",
                          vector_child_type.ToString());
  }

  // --- 2. Parse User-Provided Options ---
  ParseOptions(options, metric_type_, edge_vector_type_param_, r_, l_insert_,
               alpha_, l_search_);
  ...
  if (edge_vector_type_param_ == LmDiskannEdgeType::SAME_AS_NODE) {
    ...
    case LmDiskannEdgeType::FLOAT32:
      resolved_edge_vector_type_ = LmDiskannVectorType::FLOAT32;
      break;
    case LmDiskannEdgeType::FLOAT16:
      resolved_edge_vector_type_ = LmDiskannVectorType::FLOAT16;
      break;
    case LmDiskannEdgeType::INT8:
      resolved_edge_vector_type_ = LmDiskannVectorType::INT8;
      break;
    case LmDiskannEdgeType::FLOAT1BIT:
      // Size calculation handles this, but validation needed
      resolved_edge_vector_type_ =
          LmDiskannVectorType::UNKNOWN; // Special case handled by size calc
      break;
    default:
      throw InternalException("Unexpected LmDiskannEdgeType parameter");
    }
  }
  ...
  allocator_ = make_uniq<FixedSizeAllocator>(
      block_size_bytes_, table_io_manager_.GetIndexBlockManager());
  ...
  // --- Logging ---
  Printer::Print(
      StringUtil::Format("LM_DISKANN Index '%s': Metric=%d, Dim=%lld, R=%d, "
                         "L_insert=%d, Alpha=%.2f, L_search=%d, BlockSize=%lld",
                         name, (int)metric_type_, dimensions_, r_, l_insert_,
                         alpha_, l_search_, block_size_bytes_));
}

LmDiskannIndex::~LmDiskannIndex() =
    default; // Default destructor is likely sufficient for now

void LmDiskannIndex::ValidateParameters() {
  if (r_ == 0)
    throw BinderException("LM_DISKANN parameter R must be > 0");
  if (l_insert_ == 0)
    throw BinderException("LM_DISKANN parameter L_INSERT must be > 0");
  if (alpha_ < 1.0f)
    throw BinderException("LM_DISKANN parameter ALPHA must be >= 1.0");
  if (l_search_ == 0)
    throw BinderException("LM_DISKANN parameter L_SEARCH must be > 0");

  if (edge_vector_type_param_ == LmDiskannEdgeType::FLOAT1BIT &&
      metric_type_ != LmDiskannMetricType::COSINE) {
    throw BinderException("LM_DISKANN EDGE_TYPE 'FLOAT1BIT' is only supported "
                          "with METRIC 'COSINE'");
  }
  // Add more validation as needed (e.g., max dimensions, max R based on block
  // size)
}
...
NodeLayoutOffsets CalculateLayoutInternal(idx_t dimensions, idx_t r,
  ...
  // Align for compressed neighbors (depends on edge_vector_size_bytes
  // alignment) Assume minimal alignment needed for edge vectors unless complex
  // types used
  idx_t edge_alignment =
      (edge_vector_size_bytes > 1)
          ? std::min((idx_t)8, NextPowerOfTwo(edge_vector_size_bytes))
          : 1;
...
void LmDiskannIndex::CalculateSizesAndLayout() {
  node_vector_size_bytes_ =
      GetVectorTypeSizeBytes(node_vector_type_) * dimensions_;
  ...
  if (edge_vector_type_param_ == LmDiskannEdgeType::FLOAT1BIT) {
    ...
    edge_vector_size_bytes_ =
        GetEdgeVectorTypeSizeBytes(edge_vector_type_param_, node_vector_type_) *
        dimensions_;
  }

  // Calculate the required size and internal offsets based on the precise
  // layout
  node_layout_ = CalculateLayoutInternal(
      dimensions_, r_, node_vector_size_bytes_, edge_vector_size_bytes_);
  ...
  block_size_bytes_ =
      AlignValue(block_size_bytes_, (idx_t)Storage::SECTOR_SIZE);

  // Define DISKANN_MAX_BLOCK_SZ appropriately, e.g., #define
  // DISKANN_MAX_BLOCK_SZ (128 * 1024 * 1024)
  constexpr idx_t DISKANN_MAX_BLOCK_SZ = 128 * 1024 * 1024;
  if (block_size_bytes_ > DISKANN_MAX_BLOCK_SZ) {
    throw BinderException(
        "Calculated LM_DISKANN block size (%lld bytes) exceeds maximum allowed "
        "(%lld bytes). Reduce R or vector dimensions.",
...
void LmDiskannIndex::InitializeNewIndex(idx_t estimated_cardinality) {
  ...
  // rowid_map_ = make_uniq<ART>(db_, ART::ARTType::ROW_ID_ART, allocator_); //
  // Example rowid_map_root_ptr_ = rowid_map_->GetRoot(); // Get initial root
  // pointer
...
void LmDiskannIndex::LoadFromStorage(const IndexStorageInfo &storage_info) {
  metadata_ptr_.Set(storage_info.root_block,
                    storage_info.root_offset); // Use combined root info
  ...
  auto handle =
      buffer_manager.Pin(allocator_->GetMetaBlock(metadata_ptr_.GetBlockId()));
  ...
  if (format_version_ != LMDISKANN_CURRENT_FORMAT_VERSION) {
    ...
    throw IOException(
        "LM_DISKANN index format version mismatch. Found %d, expected %d.",
  ...
  reader.Read<LmDiskannEdgeType>(
      edge_vector_type_param_); // Read the user's choice
  ...
  if (edge_vector_type_param_ == LmDiskannEdgeType::SAME_AS_NODE) {
    ...
    case LmDiskannEdgeType::INT8:
      ...
      break;
    case LmDiskannEdgeType::FLOAT1BIT:
      resolved_edge_vector_type_ = LmDiskannVectorType::UNKNOWN;
      break;
    default:
      throw InternalException("Unexpected loaded LmDiskannEdgeType parameter");
    }
  }
  // Recalculate sizes and layout based on loaded parameters to ensure
  // consistency
  CalculateSizesAndLayout(); // This recalculates node_layout_ and checks
                             // block_size_bytes_

  // Check persisted block size against recalculated one
  idx_t expected_block_size =
      block_size_bytes_; // Already calculated in CalculateSizesAndLayout
  ...
  // Re-read block size from metadata to compare
  {
    ...
    MetadataReader reader_check(handle.GetFileBuffer(),
                                metadata_ptr_.GetOffset());
    // Seek or re-read up to block_size_bytes_ field
    reader_check.Skip(sizeof(uint8_t) + sizeof(LmDiskannMetricType) +
                      sizeof(LmDiskannVectorType) + sizeof(LmDiskannEdgeType) +
                      sizeof(idx_t) + sizeof(uint32_t) * 2 + sizeof(float) +
                      sizeof(uint32_t));
  ...
  if (persisted_block_size != expected_block_size) {
    throw IOException(
        "LM_DISKANN index load error: persisted block size (%lld) does not "
        "match calculated block size (%lld) for loaded parameters. Index may "
  ...
  if (graph_entry_point_ptr_.IsValid()) {
    ...
    graph_entry_point_rowid_ =
        -2; // Placeholder indicating valid pointer but unknown rowid
    Printer::Warning(
        "Loading entry point pointer, but rowid lookup not implemented.");
  } else {
...
void LmDiskannIndex::PersistMetadata() {
  ...
  writer.Write<LmDiskannEdgeType>(
      edge_vector_type_param_); // Persist the user's choice
  ...
  // writer.Write<IndexPointer>(rowid_map_ ? rowid_map_->GetRoot() :
  // IndexPointer());
...
IndexStorageInfo LmDiskannIndex::GetStorageInfo(const bool get_buffers) {
  // Persist any dirty metadata (including potentially updated RowID map root,
  // entry point, delete queue) if (rowid_map_) {
  //    rowid_map_root_ptr_ = rowid_map_->GetRoot();
  // }
  if (is_dirty_) { // is_dirty_ might be set by map, entry point, or queue
                   // updates
    PersistMetadata();
  }
  ...
  // Handle buffer serialization for WAL (currently not supported for custom
  // indexes)
  if (get_buffers) {
    // info.buffers.push_back(allocator_->InitSerializationToWAL()); // Likely
    // empty/no-op
  }
...
bool LmDiskannIndex::TryGetNodePointer(row_t row_id, IndexPointer &node_ptr) {
  ...
  // Printer::Warning("LmDiskannIndex::TryGetNodePointer is not implemented
  // (using placeholder)."); Placeholder: Pretend node not found until
  // implemented
...
IndexPointer LmDiskannIndex::AllocateNode(row_t row_id) {
  ...
  // if (!rowid_map_) {
  //     throw InternalException("RowID map is not initialized in
  //     AllocateNode");
  // }
  ...
  //     // Handle collision: Maybe overwrite? Delete old node first? Throw
  //     error?
  //     // For now, let's throw an error.
  //     throw ConstraintException("Cannot allocate node for row_id %lld:
  //     already exists in mapping index.", row_id);
  // }
  ...
  // if (!success) {
  //      // This shouldn't happen if the check above passed, but handle
  //      defensively allocator_->Free(new_node_ptr); // Free the newly
  //      allocated block on failure throw InternalException("Failed to insert
  //      row_id %lld into mapping index after check.", row_id);
  // }
  // is_dirty_ = true; // Mark index dirty as map has changed (needs metadata
  // persistence) Printer::Warning("LmDiskannIndex::AllocateNode is not
  // implemented (using placeholder)."); Placeholder: Return the allocated
  // pointer, but map isn't updated
...
void LmDiskannIndex::DeleteNodeFromMapAndFreeBlock(row_t row_id) {
  ...
  //         Printer::Warning("LmDiskannIndex::DeleteNodeFromMap: Failed to
  //         delete key %lld from map after finding it.", row_id);
  //     }
  // } else {
  //     // Node not found, maybe log a warning or ignore?
  //     Printer::Warning("LmDiskannIndex::DeleteNodeFromMap: Node with row_id
  //     %lld not found in map.", row_id);
  // }
  Printer::Warning("LmDiskannIndex::DeleteNodeFromMapAndFreeBlock is not "
                   "implemented (using placeholder).");
  is_dirty_ = true; // Mark dirty because map *should* have changed
}

// Gets a pinned buffer handle for a specific node's block using its
// IndexPointer
BufferHandle LmDiskannIndex::GetNodeBuffer(IndexPointer node_ptr,
                                           bool write_lock) {
  if (!allocator_) {
  ...
  // Need to use GetBlock which handles the mapping from IndexPointer to
  // block_id_t
...
const_data_ptr_t GetNodeVectorPtr(const_data_ptr_t block_ptr,
...
const row_t *GetNeighborIDsPtr(const_data_ptr_t block_ptr,
                               const NodeLayoutOffsets &layout) {
...
const_data_ptr_t GetCompressedNeighborPtr(const_data_ptr_t block_ptr,
                                          const NodeLayoutOffsets &layout,
                                          uint32_t neighbor_idx,
                                          idx_t edge_vector_size_bytes) {
  // Returns a pointer to the start of the specified neighbor's compressed
  // vector data. Note: No bounds check here for performance; caller must ensure
  // neighbor_idx is valid.
  return block_ptr + layout.compressed_neighbors +
         (neighbor_idx * edge_vector_size_bytes);
}
...
data_ptr_t GetNodeVectorPtrMutable(data_ptr_t block_ptr,
                                   const NodeLayoutOffsets &layout) {
...
row_t *GetNeighborIDsPtrMutable(data_ptr_t block_ptr,
                                const NodeLayoutOffsets &layout) {
...
data_ptr_t GetCompressedNeighborPtrMutable(data_ptr_t block_ptr,
                                           const NodeLayoutOffsets &layout,
                                           uint32_t neighbor_idx,
                                           idx_t edge_vector_size_bytes) {
  // Returns a writable pointer to the start of the specified neighbor's
  // compressed vector data. Note: No bounds check here for performance; caller
  // must ensure neighbor_idx is valid.
  return block_ptr + layout.compressed_neighbors +
         (neighbor_idx * edge_vector_size_bytes);
}
...
float LmDiskannIndex::CalculateDistance(const T_A *vec_a, const T_B *vec_b) {
  ...
  case LmDiskannMetricType::L2:
    ...
    return VectorDistance::Exec<float, float, float>(a_ptr, b_ptr, dimensions_,
                                                     VectorDistanceType::L2);
  case LmDiskannMetricType::COSINE:
    // Returns 1 - cosine_similarity. Lower is better.
    return VectorDistance::Exec<float, float, float>(
        a_ptr, b_ptr, dimensions_, VectorDistanceType::COSINE);
  case LmDiskannMetricType::IP:
    // Returns -inner_product. Lower is better.
    return -VectorDistance::Exec<float, float, float>(a_ptr, b_ptr, dimensions_,
                                                      VectorDistanceType::IP);
  default:
...
template float LmDiskannIndex::CalculateDistance<float, float>(const float *,
                                                               const float *);
template float
LmDiskannIndex::CalculateDistance<float, float16_t>(const float *,
                                                    const float16_t *);
template float LmDiskannIndex::CalculateDistance<float, int8_t>(const float *,
                                                                const int8_t *);
...
float LmDiskannIndex::CalculateApproxDistance(
  ...
  case LmDiskannVectorType::FLOAT32:
    return CalculateDistance<float, float>(
        query_ptr, reinterpret_cast<const float *>(compressed_neighbor_ptr));
  case LmDiskannVectorType::FLOAT16:
    return CalculateDistance<float, float16_t>(
        query_ptr,
        reinterpret_cast<const float16_t *>(compressed_neighbor_ptr));
  case LmDiskannVectorType::INT8:
    // Warning: Accuracy depends heavily on how INT8 was created and how
    // distance is calculated.
    return CalculateDistance<float, int8_t>(
        query_ptr, reinterpret_cast<const int8_t *>(compressed_neighbor_ptr));
  case LmDiskannVectorType::UNKNOWN: // Represents FLOAT1BIT
    if (metric_type_ == LmDiskannMetricType::COSINE) {
      // FIXME: Implement Hamming distance / popcount for FLOAT1BIT Cosine
      // approximation Calculate Hamming distance between query (needs
      // binarization) and compressed_neighbor_ptr Convert Hamming distance to
      // approximate Cosine distance.
      throw NotImplementedException(
          "Approximate distance for FLOAT1BIT not implemented.");
    } else {
      throw InternalException(
          "FLOAT1BIT edge type used with non-COSINE metric.");
    }
    break;
  default:
    throw InternalException(
        "Unsupported resolved edge vector type in CalculateApproxDistance.");
  }
}

// --- Core Algorithm Structure Placeholders ---

ErrorData LmDiskannIndex::Append(IndexLock &lock, DataChunk &entries,
  ...
  for (idx_t i = 0; i < entries.size(); ++i) {
    ...
    if (err.HasError()) {
      // Attempt to rollback or handle partial failure? Difficult without
      // transactions.
  ...
  for (idx_t i = 0; i < entries.size(); ++i) {
    ...
    try {
      ...
      if (row_id == graph_entry_point_rowid_) {
        ...
        // Need to find a new entry point later if needed, or on next
        // insert/scan
      ...
      Printer::Warning("Failed to delete node for row_id %lld: %s", row_id,
                       e.what());
    }
  }
...
ErrorData LmDiskannIndex::Insert(IndexLock &lock, DataChunk &data,
  ...
  if (node_vector_type_ == LmDiskannVectorType::FLOAT32) {
    ...
    if (node_vector_type_ == LmDiskannVectorType::FLOAT16) {
      ConvertToFloat(reinterpret_cast<const float16_t *>(input_vector_raw_ptr),
                     input_vector_float_storage.data(), dimensions_);
    } else if (node_vector_type_ == LmDiskannVectorType::INT8) {
      ConvertToFloat(reinterpret_cast<const int8_t *>(input_vector_raw_ptr),
                     input_vector_float_storage.data(), dimensions_);
    } else {
  ...
  try {
    new_node_ptr =
        AllocateNode(row_id); // Allocates block and updates map (placeholder)
  } catch (std::exception &e) {
    return ErrorData(e);
  }
  ...
  memcpy(LmDiskannNodeAccessors::GetNodeVectorPtrMutable(new_node_data,
                                                         node_layout_),
  ...
  if (entry_point_row_id == NumericLimits<row_t>::Maximum()) {
    ...
    try {
      ...
      // DeleteNodeFromMapAndFreeBlock(row_id); // Risky if map isn't
      // transactional
      return ErrorData(StringUtil::Format(
          "Failed to connect neighbors for node %lld: %s", row_id, e.what()));
    }
  }
...
static bool CompressVectorForEdge(const float *input_float_ptr,
                                  data_ptr_t dest_ptr, idx_t dimensions,
                                  LmDiskannVectorType resolved_edge_type) {
...
void LmDiskannIndex::FindAndConnectNeighbors(
  ...
  LmDiskannScanState search_state(query_vec_handle, l_insert_,
                                  l_insert_); // Use l_insert for k and L
  PerformSearch(search_state, false);         // Find approximate neighbors

  // Candidates are in search_state.candidates (priority queue) and
  // search_state.visited (set) We need the combined set of visited nodes as
  // potential neighbors.
  ...
  duckdb::unordered_set<row_t>
      added_neighbor_ids; // Track added to avoid duplicates

  // Iterate through visited list (more reliable than candidates queue)
  // Need to actually store visited nodes with distances during search for
  // this... Alternative: Use top_candidates from search if PerformSearch stored
  // them? Let's use the priority queue content for now as an approximation.
  ...
  row_t *new_neighbor_ids = LmDiskannNodeAccessors::GetNeighborIDsPtrMutable(
      new_node_data, node_layout_);
  data_ptr_t new_compressed_neighbors_base =
      LmDiskannNodeAccessors::GetCompressedNeighborPtrMutable(
          new_node_data, node_layout_, 0, edge_vector_size_bytes_);
  ...
  for (const auto &candidate : potential_neighbors) {
    if (current_neighbor_count >= r_)
      break; // Stop if we have R neighbors

    row_t neighbor_rowid = candidate.second;
    if (neighbor_rowid == new_node_rowid)
      continue; // Don't add self

    IndexPointer neighbor_ptr;
    if (!TryGetNodePointer(neighbor_rowid, neighbor_ptr))
      continue; // Skip if neighbor deleted

    // Read neighbor's full vector
    auto neighbor_handle = GetNodeBuffer(neighbor_ptr);
    auto neighbor_block_data = neighbor_handle.Ptr();
    auto neighbor_node_vec_ptr_raw = LmDiskannNodeAccessors::GetNodeVectorPtr(
        neighbor_block_data, node_layout_);

    // Convert neighbor vector to float for pruning calculations
    // FIXME: Handle different node_vector_type_
    if (node_vector_type_ != LmDiskannVectorType::FLOAT32) {
      throw NotImplementedException("FindAndConnectNeighbors for non-FLOAT32 "
                                    "node vectors not implemented.");
    }
    const float *neighbor_node_vec_ptr_float =
        reinterpret_cast<const float *>(neighbor_node_vec_ptr_raw);

    // --- Robust Pruning Check (Simplified from C code's diskAnnReplaceEdgeIdx)
    // --- Check if any *existing* neighbor in the new node's list prunes this
    // candidate
    bool pruned = false;
    float dist_new_to_candidate = CalculateDistance<float, float>(
        new_node_vector_float, neighbor_node_vec_ptr_float);
    ...
    for (uint16_t i = 0; i < current_neighbor_count; ++i) {
      ...
      if (!TryGetNodePointer(existing_neighbor_id, existing_neighbor_ptr))
        continue;
      ...
      auto existing_neighbor_vec_ptr_raw =
          LmDiskannNodeAccessors::GetNodeVectorPtr(existing_neighbor_block_data,
                                                   node_layout_);
      // FIXME: Handle non-FLOAT32 node_vector_type_
      const float *existing_neighbor_vec_ptr_float =
          reinterpret_cast<const float *>(existing_neighbor_vec_ptr_raw);

      float dist_existing_to_candidate = CalculateDistance<float, float>(
          existing_neighbor_vec_ptr_float, neighbor_node_vec_ptr_float);
    ...
    if (pruned)
      continue; // This candidate is pruned by an existing neighbor

    // Add the neighbor
    new_neighbor_ids[current_neighbor_count] = neighbor_rowid;
    // Compress neighbor vector and store it
    data_ptr_t dest_compress_ptr =
        new_compressed_neighbors_base +
        current_neighbor_count * edge_vector_size_bytes_;
    if (!CompressVectorForEdge(neighbor_node_vec_ptr_float, dest_compress_ptr,
                               dimensions_, resolved_edge_vector_type_)) {
      throw InternalException(
          "Failed to compress neighbor vector for new node.");
    }
    added_neighbor_ids.insert(neighbor_rowid); // Track added neighbors
    current_neighbor_count++;
  }
  LmDiskannNodeAccessors::SetNeighborCount(new_node_data,
                                           current_neighbor_count);
  ...
  vector<float> new_node_compressed_storage(
      edge_vector_size_bytes_); // Temp storage for compressed new vec
  data_ptr_t new_node_compressed_ptr = nullptr;
  if (resolved_edge_vector_type_ != LmDiskannVectorType::FLOAT32) {
    if (!CompressVectorForEdge(new_node_vector_float,
                               new_node_compressed_storage.data(), dimensions_,
                               resolved_edge_vector_type_)) {
      throw InternalException(
          "Failed to compress new node vector for reciprocal edges.");
    }
    new_node_compressed_ptr = new_node_compressed_storage.data();
  } else {
    new_node_compressed_ptr =
        (data_ptr_t)new_node_vector_float; // Use original if float
  }
  ...
  for (row_t neighbor_rowid : added_neighbor_ids) {
    ...
    auto neighbor_handle =
        GetNodeBuffer(neighbor_ptr, true); // Get writable buffer
    auto neighbor_data = neighbor_handle.Ptr();

    // --- Robust Pruning on Neighbor ---
    // This logic needs to be carefully translated from
    // diskAnnReplaceEdgeIdx/diskAnnPruneEdges It involves calculating distances
    // between the neighbor and its existing neighbors, comparing them with
    // distances involving the new node, and potentially replacing an existing
    // edge with the new edge (neighbor -> new_node).
    ...
    uint16_t neighbor_current_count =
        LmDiskannNodeAccessors::GetNeighborCount(neighbor_data);
    if (neighbor_current_count < r_) {
      row_t *neighbor_neighbor_ids =
          LmDiskannNodeAccessors::GetNeighborIDsPtrMutable(neighbor_data,
                                                           node_layout_);
      data_ptr_t neighbor_compressed_base =
          LmDiskannNodeAccessors::GetCompressedNeighborPtrMutable(
              neighbor_data, node_layout_, 0, edge_vector_size_bytes_);

      neighbor_neighbor_ids[neighbor_current_count] = new_node_rowid;
      memcpy(neighbor_compressed_base +
                 neighbor_current_count * edge_vector_size_bytes_,
             new_node_compressed_ptr, edge_vector_size_bytes_);
      LmDiskannNodeAccessors::SetNeighborCount(neighbor_data,
                                               neighbor_current_count + 1);
      neighbor_handle.SetModified();
    } else {
      // FIXME: Need pruning logic to decide if new node replaces an existing
      // edge
      Printer::Warning("Neighbor %lld is full (R=%d), skipping reciprocal edge "
                       "addition due to missing pruning logic.",
                       neighbor_rowid, r_);
    }
  }

  // throw NotImplementedException("LmDiskannIndex::FindAndConnectNeighbors not
  // fully implemented (Robust Pruning)");
}
...
LmDiskannIndex::InitializeScan(ClientContext &context,
  ...
  if (query_dims != dimensions_) {
    throw BinderException(
        "Query vector dimension (%llu) does not match index dimension (%llu).",
        query_dims, dimensions_);
  }

  // 2. Create scan state
  auto scan_state = make_uniq<LmDiskannScanState>(
      query_vector, k, l_search_); // Use index's L_search
  ...
  if (start_node_id != NumericLimits<row_t>::Maximum()) {
    ...
    if (TryGetNodePointer(start_node_id, start_ptr)) {
      try {
        ...
        auto node_vec_ptr =
            LmDiskannNodeAccessors::GetNodeVectorPtr(block_data, node_layout_);
        // Need to handle node vector type here for approx distance!
        float approx_dist;
        if (node_vector_type_ == LmDiskannVectorType::FLOAT32) {
          approx_dist = CalculateApproxDistance(scan_state->query_vector_ptr,
                                                node_vec_ptr);
        } else {
          // FIXME: Need conversion or specialized approx distance for non-float
          // node vectors
          throw NotImplementedException("Initial scan distance for non-FLOAT32 "
                                        "node vectors not implemented.");
        }
        ...
        Printer::Warning("Failed to initialize scan with start node %lld: %s",
                         start_node_id, e.what());
      }
    } else {
      // Start node not found (maybe deleted?), try random?
      Printer::Warning(
          "Persisted entry point node %lld not found, trying random.",
          start_node_id);
      start_node_id = GetRandomNodeID(); // Placeholder
      if (start_node_id != NumericLimits<row_t>::Maximum() &&
          TryGetNodePointer(start_node_id, start_ptr)) {
        // Try again with random node
        try {
          auto handle = GetNodeBuffer(start_ptr);
          auto block_data = handle.Ptr();
          auto node_vec_ptr = LmDiskannNodeAccessors::GetNodeVectorPtr(
              block_data, node_layout_);
          ...
          if (node_vector_type_ == LmDiskannVectorType::FLOAT32) {
            ...
            throw NotImplementedException(
                "Initial scan distance for non-FLOAT32 node vectors not "
                "implemented.");
          }
          scan_state->candidates.push({approx_dist, start_node_id});
        } catch (std::exception &e) {
          Printer::Warning(
              "Failed to initialize scan with random node %lld: %s",
              start_node_id, e.what());
        }
      }
    }
...
idx_t LmDiskannIndex::Scan(IndexScanState &state, Vector &result) {
  ...
  for (const auto &candidate : scan_state.top_candidates) {
    if (output_count < STANDARD_VECTOR_SIZE && output_count < scan_state.k) {
      result_data[output_count++] =
          candidate.second; // candidate.second is row_t
    } else {
      break;
    }
  }
...
void LmDiskannIndex::PerformSearch(LmDiskannScanState &scan_state,
  ...
  while (!scan_state.candidates.empty()) {
    ...
    if (find_exact_distances &&
        scan_state.top_candidates.size() >= scan_state.k) {
    ...
    float candidate_dist_approx =
        scan_state.candidates.top()
            .first; // Approx distance used for queue ordering
    ...
    if (!TryGetNodePointer(candidate_row_id, node_ptr)) {
      ...
      Printer::Warning(
          "Node %lld not found in map during search (likely deleted).",
          candidate_row_id);
      continue;
    }
    ...
    try {
      ...
      Printer::Warning("Failed to read block for node %lld during search: %s",
                       candidate_row_id, e.what());
    ...
    if (find_exact_distances) {
      const_data_ptr_t node_vec_raw_ptr =
          LmDiskannNodeAccessors::GetNodeVectorPtr(block_data, node_layout_);
      float exact_dist;
      // Calculate exact distance based on node type
      switch (node_vector_type_) {
      case LmDiskannVectorType::FLOAT32:
        exact_dist = CalculateDistance<float, float>(
            scan_state.query_vector_ptr,
            reinterpret_cast<const float *>(node_vec_raw_ptr));
        break;
      case LmDiskannVectorType::FLOAT16:
        exact_dist = CalculateDistance<float, float16_t>(
            scan_state.query_vector_ptr,
            reinterpret_cast<const float16_t *>(node_vec_raw_ptr));
        break;
      case LmDiskannVectorType::INT8:
        exact_dist = CalculateDistance<float, int8_t>(
            scan_state.query_vector_ptr,
            reinterpret_cast<const int8_t *>(node_vec_raw_ptr));
        break;
      default:
        throw InternalException(
            "Unsupported node vector type for exact distance calculation.");
      }

      // Add to top_candidates and keep it sorted and capped at size k
      if (scan_state.top_candidates.size() < scan_state.k ||
          exact_dist < scan_state.top_candidates.back().first) {
        scan_state.top_candidates.push_back({exact_dist, candidate_row_id});
        std::sort(scan_state.top_candidates.begin(),
                  scan_state.top_candidates.end()); // Keep sorted
        if (scan_state.top_candidates.size() > scan_state.k) {
    ...
    const row_t *neighbor_ids =
        LmDiskannNodeAccessors::GetNeighborIDsPtr(block_data, node_layout_);
    ...
    for (uint16_t i = 0; i < ncount; ++i) {
      ...
      const_data_ptr_t compressed_ptr =
          LmDiskannNodeAccessors::GetCompressedNeighborPtr(
              block_data, node_layout_, i, edge_vector_size_bytes_);

      // 9. Calculate approximate distance
      float approx_dist =
          CalculateApproxDistance(scan_state.query_vector_ptr, compressed_ptr);

      // 10. Add to candidates queue if promising
      // Only add if it's better than the worst in the queue OR the queue isn't
      // full
      if (scan_state.candidates.size() < scan_state.l_search ||
          approx_dist < scan_state.candidates.top().first) {
...
void LmDiskannIndex::EnqueueDeletion(row_t deleted_row_id) {
  ...
  //    IndexPointer new_queue_entry_ptr =
  //    allocator_->New(DELETE_QUEUE_ENTRY_SIZE); // Need allocator variant? For
  //    now, assume we use the main allocator and block_size_bytes_
  ...
  is_dirty_ = true;
  // Printer::Warning("LmDiskannIndex::EnqueueDeletion not fully implemented
  // (using main allocator).");
}
...
void LmDiskannIndex::ProcessDeletionQueue() {
  ...
  if (delete_queue_head_ptr_.IsValid()) {
    Printer::Warning("LmDiskannIndex::ProcessDeletionQueue: Processing "
                     "deferred deletions is not implemented.");
...
row_t LmDiskannIndex::GetEntryPoint() {
  ...
  if (graph_entry_point_rowid_ != NumericLimits<row_t>::Maximum()) {
    ...
    if (TryGetNodePointer(graph_entry_point_rowid_, ptr_check)) {
      ...
      Printer::Warning("Cached entry point %lld deleted.",
                       graph_entry_point_rowid_);
      graph_entry_point_ptr_.Clear();
      graph_entry_point_rowid_ = NumericLimits<row_t>::Maximum();
  ...
  if (graph_entry_point_ptr_.IsValid()) {
    ...
    Printer::Warning("Entry point pointer is valid, but rowid lookup not "
                     "implemented. Cannot use as entry point yet.");
...
row_t LmDiskannIndex::GetRandomNodeID() {
  ...
  // Printer::Warning("LmDiskannIndex::GetRandomNodeID not implemented,
  // returning invalid rowid.");
  return NumericLimits<row_t>::Maximum();
}
