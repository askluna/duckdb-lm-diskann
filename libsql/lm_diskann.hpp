...
private:
    ...
    static idx_t GetEdgeVectorTypeSizeBytes(LMDiskannEdgeType type, LMDiskannVectorType node_type);
    ...
    // Calculates distance between full query (assumed float) and compressed neighbor
    ...
    // --- Deletion Helpers ---
    void EnqueueDeletion(row_t deleted_row_id); // Adds row_id to the persistent delete queue
    void ProcessDeletionQueue(); // Processes the queue during Vacuum

    // --- Entry Point Helpers ---
    row_t GetEntryPoint(); // Gets a valid entry point row_id (persisted or random)
    void SetEntryPoint(row_t row_id, IndexPointer node_ptr); // Updates the persisted entry point
    row_t GetRandomNodeID(); // Placeholder for getting a random node (needs map iteration)
