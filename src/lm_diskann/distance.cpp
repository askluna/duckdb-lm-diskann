...
float ComputeApproxSimilarityTernary(
  ...
  if (!query_float_ptr || !neighbor_planes.IsValid() || dimensions == 0) {
    throw std::runtime_error(
        "Invalid arguments for ComputeApproxSimilarityTernary");
  }
  ...
  const size_t words_per_vector =
      neighbor_planes.words_per_plane; // Use size from view
  std::vector<uint64_t> query_pos_plane_vec(words_per_vector);
  std::vector<uint64_t> query_neg_plane_vec(words_per_vector);

  EncodeTernary(query_float_ptr, query_pos_plane_vec.data(),
                query_neg_plane_vec.data(), dimensions);
  ...
  int64_t raw_score = dot_kernel(
      query_pos_plane_vec.data(), query_neg_plane_vec.data(),
