...
struct LMDiskannDBState {
     ...
     */
    LMDiskannDBState(AttachedDatabase &db_ref, TableIOManager &io_manager, const LogicalType &type_ref)
	...
	LogicalType indexed_column_type; // The full logical type of the indexed column (e.g., ARRAY(FLOAT, 128)).
	unique_ptr<FixedSizeAllocator> allocator; // Manages disk blocks for index nodes.
	IndexPointer metadata_ptr;       // Pointer to the block holding persistent index metadata.
};
...
public:
    /**
     * @brief Type name used for registration and in CREATE INDEX ... USING LM_DISKANN.
     */
    static constexpr const char *TYPE_NAME = "LM_DISKANN";

    /**
     * @brief Constructor: Initializes parameters, storage, and loads/creates the index.
     * @param name Index name.
     * @param index_constraint_type Type of index constraint (UNIQUE, PRIMARY_KEY, FOREIGN_KEY, NONE).
     * @param column_ids List of physical column identifiers bound to the index expressions.
     * @param table_io_manager The table's IO manager.
     * @param unbound_expressions The unbound expressions used to create the index.
    ...
    /** @brief Verifies index integrity and returns a string representation (verification TBD). */
    ...
    idx_t GetEdgeVectorSizeBytes() const { return node_layout_.ternary_edge_size_bytes; }
    ...
    LMDiskannVectorType GetNodeVectorType() const { return config_.node_vector_type; }
...
private:
     ...
     */
    friend void PerformSearch(LMDiskannScanState &scan_state, LMDiskannIndex &index, const LMDiskannConfig& config, bool find_exact_distances);
    ...
    /** @brief Tries to find the node pointer for a given RowID using the current map. */
    ...
    /** @brief Applies robust pruning to a node's potential neighbors. Updates the node's block. */
    void RobustPrune(row_t node_rowid, IndexPointer node_ptr, std::vector<std::pair<float, row_t>>& candidates);

    // --- Entry Point Helpers --- //
    /** @brief Gets the current valid entry point RowID (might involve lookup or random selection). */
    row_t GetEntryPoint();
    /** @brief Sets the current entry point pointer and cached RowID. */
    void SetEntryPoint(row_t row_id, IndexPointer node_ptr);
    /** @brief Gets a random RowID from the current set of nodes (placeholder using map). */
    ...
    // --- Distance Helpers --- //
    /** @brief Calculates approximate distance using ternary compressed vector. Uses config_. */
    ...
    void CompressVectorForEdge(const float* input_vector, data_ptr_t output_compressed_vector);
    ...
    float CalculateExactDistance(const T_QUERY *query_ptr, const_data_ptr_t node_vector_ptr);
    /** @brief Converts raw node vector to float. Uses config_. */
    void ConvertNodeVectorToFloat(const_data_ptr_t raw_node_vector, float* float_vector_out);
