...
HWY_ALIGN_MAX const uint8_t nibble_popcount_lut_data[16] = {
    /* 0 */ 0, /* 1 */ 1, /* 2 */ 1, /* 3 */ 2,
    /* 4 */ 1, /* 5 */ 2, /* 6 */ 2, /* 7 */ 3,
    /* 8 */ 1, /* 9 */ 2, /* A */ 2, /* B */ 3,
    /* C */ 2, /* D */ 3, /* E */ 3, /* F */ 4};
...
inline void EncodeTernary(const Scalar *src, uint64_t *pos, uint64_t *neg, size_t dims) {
	...
	hn::ScalableTag<Scalar> d_scalar;
	using VScalar = hn::Vec<decltype(d_scalar)>;
	const size_t lanes = hn::Lanes(d_scalar);
	auto zero_vec = hn::Zero(d_scalar);

	for (size_t d_base = 0; d_base < dims; d_base += lanes) {
		const size_t num_elements_to_process = std::min(lanes, dims - d_base);

		auto current_elements_mask = hn::FirstN(d_scalar, num_elements_to_process);
		// Load only the valid elements for this iteration.
		VScalar scalar_vec = hn::MaskedLoad(current_elements_mask, d_scalar, src + d_base);

		// Perform comparisons. MaskedLoad ensures out-of-bounds lanes are zero,
		// so they won't satisfy Gt/Lt(zero).
		auto positive_simd_mask = hn::Gt(scalar_vec, zero_vec);
		auto negative_simd_mask = hn::Lt(scalar_vec, zero_vec);
		...
		uint64_t pos_bits_for_vector_lanes = hn::BitsFromMask(d_scalar, positive_simd_mask);
		uint64_t neg_bits_for_vector_lanes = hn::BitsFromMask(d_scalar, negative_simd_mask);
...
// HWY_TARGET_INCLUDE is defined at the top of the file (line 87)
// It points to this file: "src/lm_diskann/core/ternary_quantization.hpp"
// Now include foreach_target.h to set up multi-target compilation for the kernel.
#include "hwy/foreach_target.h"
// Include highway.h again here; this one will be processed for each target by foreach_target.h
#include "hwy/highway.h"
...
HWY_ATTR hwy::HWY_NAMESPACE::Vec<D_U8> PopCountBytesUsingLUT(D_U8 d_u8, hwy::HWY_NAMESPACE::Vec<D_U8> bytes_vec) {
	...
	auto lut_lanes = hwy::HWY_NAMESPACE::LoadDup128(d_u8, diskann::core::nibble_popcount_lut_data);
...
HWY_ATTR HWY_INLINE hwy::HWY_NAMESPACE::Vec<D_U64>
SumBytePopcountsToUint64Lanes(D_U64 d_u64, D_U8 d_u8, hwy::HWY_NAMESPACE::Vec<D_U8> byte_popcounts_vec) {
	...
	// Use MaxLanes(descriptor) for current target's max lanes for stack arrays if appropriate
	HWY_ALIGN uint64_t lane_sums_storage[hwy::HWY_NAMESPACE::MaxLanes(D_U64())];

	HWY_ALIGN uint8_t byte_counts_storage[hwy::HWY_NAMESPACE::MaxLanes(D_U8())];
...
HWY_ATTR int64_t HighwayTernaryDotKernel(const uint64_t *HWY_RESTRICT qp, const uint64_t *HWY_RESTRICT qn,
	...
	VU64 acc_total = hwy::HWY_NAMESPACE::Zero(d_u64);
	...
	for (; i + lanes <= words; i += lanes) {
		...
		VU64 term_sum = pop_qp_vp;
		term_sum = hwy::HWY_NAMESPACE::Sub(term_sum, pop_qp_vn);
		term_sum = hwy::HWY_NAMESPACE::Sub(term_sum, pop_qn_vp);
		term_sum = hwy::HWY_NAMESPACE::Add(term_sum, pop_qn_vn);
...
// Export the kernel function from this namespace.
// The name used here must match the first argument to HWY_EXPORT_AND_DYNAMIC_DISPATCH.
HWY_EXPORT(HighwayTernaryDotKernel);
...
// Ensure Highway macros are available for the HWY_ONCE block.
#include "hwy/highway.h" // Explicitly include for HWY_EXPORT_AND_DYNAMIC_DISPATCH

namespace diskann {
namespace core {

// This macro defines a dispatcher function named "HighwayTernaryDotKernel"
// in the current namespace (diskann::core). This dispatcher will call
// the appropriate target-specific version of "HighwayTernaryDotKernel"
// from the HWY_NAMESPACE.
HWY_EXPORT_AND_DYNAMIC_DISPATCH(HighwayTernaryDotKernel);

inline dot_fun_t GetDotKernel() {
	// Return the address of the dispatcher function.
	// The dispatcher has the same signature as the kernel.
	return &HighwayTernaryDotKernel;
}
