...
public:
  ...
  void InitializeEmptyGraph(
      const LmDiskannConfig
          &config_param, // Passed for potential re-init scenarios
  ...
  bool AddNode(
      ...
      common::IndexPointer
          &node_ptr_out // Output: pointer to the new node block
      ) override;

  bool GetNodeVector(
      common::IndexPointer node_ptr, float *vector_out, // Output buffer
      common::idx_t dimensions // Expected dimensions, checked against config_
  ) const override;
  ...
  void RobustPrune(
      ...
      const LmDiskannConfig
          &config_param // Passed for pruning params (R, alpha)
      ) override;
   ...
   */
  common::row_t
  GetRandomNodeID(common::RandomEngine &engine); // Not virtual, no override
...
private:
  ...
  float CalculateDistanceInternal(common::IndexPointer node1_ptr,
                                  common::IndexPointer node2_ptr) const;
  float CalculateDistanceInternal(const float *vec1, const float *vec2,
                                  common::idx_t dimensions) const;
  float CalculateDistanceInternal(common::IndexPointer node_ptr,
...
public:
  ...
  static TernaryPlanesView GetNeighborTernaryPlanes(
