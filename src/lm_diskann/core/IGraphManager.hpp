...
class IGraphManager {
	...
	virtual void
	InitializeEmptyGraph(const LmDiskannConfig &config,
	                     common::IndexPointer &entry_point_ptr_out, // Output: initial (invalid) entry point ptr
	                     common::row_t &entry_point_rowid_out       // Output: initial (invalid) entry point rowid
	                     ) = 0;

	// Allocates space for a new node and stores its vector data.
	virtual bool AddNode(common::row_t row_id,
	...
	virtual bool GetNeighbors(common::IndexPointer node_ptr, std::vector<common::row_t> &neighbor_row_ids_out) const = 0;

	// Prunes a list of candidate neighbors for a node and updates its
	// connections.
	virtual void RobustPrune(common::IndexPointer node_to_connect,          // The node whose neighbors are
	                                                                        // being pruned
	...
	virtual void SetEntryPoint(common::IndexPointer node_ptr, common::row_t row_id) = 0;
