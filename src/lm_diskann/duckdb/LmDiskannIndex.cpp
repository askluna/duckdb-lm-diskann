...
#include "../core/Orchestrator.hpp" // Include Orchestrator
...
core::LmDiskannConfig LmDiskannIndex::ParseOptions(
  ...
  core::LmDiskannConfig
      config; // Starts with default values from core::LmDiskannConfigDefaults
  ...
  for (const auto &entry : options) {
    ...
    if (key_upper ==
        core::LmDiskannOptionKeys::METRIC) { // Use core::LmDiskannOptionKeys
...
// is_dirty_ is now managed by Orchestrator
{
  ...
  // Store these temporarily if Orchestrator's config doesn't directly hold them
  // or if managers need them separately during construction before
  // Orchestrator.
  core::NodeLayoutOffsets local_node_layout =
      CalculateLayoutInternal(local_config);
  idx_t local_block_size_bytes =
      ::duckdb::AlignValue<idx_t, ::duckdb::Storage::SECTOR_SIZE>(
          local_node_layout.total_node_size);
  ...
  // Create Orchestrator and its dependencies
  // These are placeholder creations. Actual managers will need proper
  // construction. The config passed to Orchestrator should be the one it owns.
  // For now, Orchestrator takes IndexConfig by const ref, so local_config is
  // fine. If Orchestrator takes IndexConfig by unique_ptr, we'd move it.
  ...
  std::unique_ptr<core::IStorageManager> storage_manager_ptr =
      std::make_unique<core::StorageManager>(buffer_manager, local_config,
                                             local_node_layout);
  std::unique_ptr<core::IGraphManager> graph_manager_ptr =
      std::make_unique<core::GraphManager>(
          buffer_manager, local_block_size_bytes); // Simplified for now
  std::unique_ptr<core::ISearcher> searcher_ptr =
      std::make_unique<core::Searcher>(
          local_config,
  ...
  orchestrator_ = std::make_unique<core::Orchestrator>(
      std::move(storage_manager_ptr), std::move(graph_manager_ptr),
      std::move(searcher_ptr), std::move(shadow_service_ptr),
      local_config // Pass the config
  );

  // Remove LmDiskannIndex's direct ownership of these if Orchestrator now
  // manages them this->config_ = local_config; // Orchestrator has a copy
  // this->node_layout_ = local_node_layout; // Orchestrator's config/managers
  // handle this this->block_size_bytes_ = local_block_size_bytes; //
  // Orchestrator's config/managers handle this this->node_manager_ = ... //
  // Orchestrator has graph_manager_ this->graph_operations_ = ... // Logic
  ...
  if (!storage_info.IsValid()) {
    ...
    // Initialize a brand new index VIA ORCHESTRATOR
    // InitializeNewIndex(estimated_cardinality); // OLD CALL
    orchestrator_->InitializeIndex(estimated_cardinality);
  } else {
    // Load an existing index from storage VIA ORCHESTRATOR
    ...
    orchestrator_->LoadIndex(this->index_data_path_);
  }
  ...
  ::duckdb::Printer::Print(::duckdb::StringUtil::Format(
      ...
      LmDiskannVectorTypeToString(local_config.node_vector_type),
      local_config.dimensions, local_config.r, local_config.l_insert,
      local_config.alpha, local_config.l_search, local_block_size_bytes));
}
...
void LmDiskannIndex::PublicMarkDirty(bool dirty_state) {
  // This logic is now primarily in Orchestrator::LoadIndex or its managers.
  // LmDiskannIndex might still need to translate IndexStorageInfo if
  // Orchestrator or its StorageManager cannot directly use it.
  ...
  // // is_dirty_ = false; // Orchestrator handles this

  // // For now, this method in LmDiskannIndex can be a no-op or ensure
  // // Orchestrator did its job.
  if (orchestrator_) {
    // orchestrator_->LoadIndex(this->index_data_path_); // This is now called
    // in constructor
    this->graph_entry_point_rowid_ = orchestrator_->GetGraphEntryPointRowId();
    // this->graph_entry_point_ptr_ = orchestrator_->GetGraphEntryPointPtr();
  }
  std::cout << "LmDiskannIndex::PublicMarkDirty called (now delegates to "
               "Orchestrator primarily)"
            << std::endl;
}
...
::duckdb::ErrorData LmDiskannIndex::Append(::duckdb::IndexLock &lock,
  ...
  // is_dirty_ flag is managed by the orchestrator during Insert calls
  return ::duckdb::ErrorData();
}
...
void LmDiskannIndex::CommitDrop(::duckdb::IndexLock &index_lock) {
  if (orchestrator_) {
    orchestrator_->HandleCommitDrop(); // Delegate drop logic
  }
  // Clear any remaining DuckDB-specific state if needed
  db_state_.metadata_ptr.Clear();
  // orchestrator_ unique_ptr will handle deletion of components
}
...
void LmDiskannIndex::Delete(::duckdb::IndexLock &lock,
  ...
  if (!orchestrator_) {
    ...
    throw ::duckdb::InternalException(
        "Orchestrator is not initialized in LmDiskannIndex::Delete");
  }

  for (idx_t i = 0; i < entries.size(); ++i) {
    ::duckdb::row_t row_id = row_ids_data[i];
    try {
      // Delegate the entire deletion logic to the orchestrator
      orchestrator_->Delete(row_id);
      // is_dirty flag is now managed by the orchestrator internally

    } catch (const ::duckdb::NotImplementedException &e) {
      // Re-throw specific exceptions if needed
      throw;
    } catch (const std::exception &e) {
      // Log or handle errors reported by the orchestrator
      ::duckdb::Printer::Print(::duckdb::StringUtil::Format(
          "Warning: Orchestrator failed to delete node for row_id %lld: %s",
          row_id, e.what()));
  ...
  // No need to set is_dirty_ here, orchestrator handles it.
}
...
::duckdb::ErrorData LmDiskannIndex::Insert(::duckdb::IndexLock &lock,
  ...
  if (!orchestrator_) {
    return ::duckdb::ErrorData(
        "Orchestrator is not initialized in LmDiskannIndex::Insert");
  }
  ...
  // Determine vector type and dimensions from Orchestrator's config
  const auto &config = orchestrator_->GetConfig();
  idx_t dimensions = config.dimensions;
  auto node_vector_type = config.node_vector_type;

  // Prepare float vector (Orchestrator::Insert expects float*)
  ::duckdb::vector<float> input_vector_float_storage(
  ...
  try {
    if (node_vector_type == core::LmDiskannVectorType::FLOAT32) {
      ...
      if (::duckdb::ArrayType::GetChildType(db_state_.indexed_column_type)
              .id() != ::duckdb::LogicalTypeId::FLOAT) {
        return ::duckdb::ErrorData(
            "Type mismatch: Config expects FLOAT32 but input is not FLOAT.");
      }
      ...
      if (::duckdb::ArrayType::GetChildType(db_state_.indexed_column_type)
        ...
        return ::duckdb::ErrorData(
            "Type mismatch: Config expects INT8 but input is not TINYINT.");
      }
      // Use the existing conversion helper (which should ideally move to a
      // common place or be part of Orchestrator/GraphManager)
      ...
      return ::duckdb::ErrorData(
          "Unsupported node vector type derived from config.");
    }
    ...
    // Delegate the core insertion logic to the orchestrator
    orchestrator_->Insert(input_vector_float_ptr, dimensions, row_id);
    // is_dirty_ flag is managed by orchestrator

    return ::duckdb::ErrorData(); // Success

  } catch (const std::exception &e) {
    // Catch errors from orchestrator or conversion
...
idx_t LmDiskannIndex::GetInMemorySize() {
  if (orchestrator_) {
    return orchestrator_->GetInMemorySize();
  }
  return 0; // Or throw an exception if orchestrator_ should always be valid
}
...
void LmDiskannIndex::Vacuum(::duckdb::IndexLock &state) {
  if (orchestrator_) {
    orchestrator_->PerformVacuum(); // Delegate vacuum logic
  }
  ::duckdb::Printer::Print(
      "LmDiskannIndex::Vacuum called (delegated to Orchestrator).");
}
...
LmDiskannIndex::InitializeScan(::duckdb::ClientContext &context,
  ...
  if (!orchestrator_) {
    throw ::duckdb::InternalException(
        "Orchestrator not initialized in InitializeScan");
  }
  const auto &config =
      orchestrator_->GetConfig(); // Get config via orchestrator
  ...
  auto scan_state = ::duckdb::make_unique<LmDiskannScanState>(query_vector, k,
                                                              config.l_search);

  // PerformSearch is no longer called here.
  // Scan() will call Orchestrator::Search().
...
idx_t LmDiskannIndex::Scan(::duckdb::IndexScanState &state,
  ...
  if (!orchestrator_) {
    throw ::duckdb::InternalException("Orchestrator not initialized in Scan");
  }
  ...
  if (!scan_state.query_vector_ptr) {
    throw ::duckdb::InternalException(
        "Query vector pointer not set in scan state");
  }

  try {
    // Delegate search to orchestrator. Results are populated in
    // scan_state.result_row_ids.
    orchestrator_->Search(scan_state.query_vector_ptr, scan_state.k,
                          scan_state.result_row_ids, scan_state.l_search);

    // Copy results from the state vector to the DuckDB output vector
    for (const auto &row_id : scan_state.result_row_ids) {
      if (output_count <
          STANDARD_VECTOR_SIZE) { // Check against DuckDB vector size
    ...
    if (output_count > 0 && output_count <= scan_state.result_row_ids.size()) {
      scan_state.result_row_ids.erase(scan_state.result_row_ids.begin(),
                                      scan_state.result_row_ids.begin() +
                                          output_count);
    } else {
    ...
    // Handle errors from orchestrator search
    throw ::duckdb::IOException(::duckdb::StringUtil::Format(
        "Error during LM_DISKANN scan: %s", e.what()));
  }
...
void LmDiskannIndex::InitializeNewIndex(idx_t estimated_cardinality) {
  // This logic is now primarily in Orchestrator::InitializeIndex
  // or delegated to its managers.
  // LmDiskannIndex might still need to do some high-level DuckDB setup
  // if not handled by Orchestrator's dependencies.
  ...
  // // Let Orchestrator handle this.
  // // is_dirty_ = false; // If metadata persistence counts as clean

  // // For now, this method in LmDiskannIndex can be a no-op or ensure
  // // Orchestrator did its job.
  if (orchestrator_) {
    // orchestrator_->InitializeIndex(estimated_cardinality); // This is now
    // called in constructor Entry point might be accessible from orchestrator
    // if needed here this->graph_entry_point_rowid_ =
    // orchestrator_->GetGraphEntryPointRowId(); // REMOVE: Member doesn't exist
    // this->graph_entry_point_ptr_ = orchestrator_->GetGraphEntryPointPtr(); //
 ...
 */
void LmDiskannIndex::LoadFromStorage(
