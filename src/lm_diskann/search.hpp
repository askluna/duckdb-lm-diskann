...
/**
 * @brief Performs the beam search algorithm to find approximate nearest neighbors.
 *
 * @details This function implements the core graph traversal logic for DiskANN.
 * It starts from one or more entry points, explores the graph using a priority queue
 * (beam) of candidates, and maintains a set of visited nodes to avoid redundant work.
 * The search depth and width are controlled by L_search (from config).
 *
 * @param scan_state The current scan state, containing the query vector, candidate queue, visited set, etc.
 * @param index The LMDiskannIndex instance, providing access to node data and configuration.
 * @param config The index configuration (L_search, metric, dimensions, etc.).
 * @param find_exact_distances If true, calculate exact distances for the final top-k results (expensive).
 */
